...
function toBase64(str) {
  ...
  let i2 = 0;
  while (i2 < str.length) {
    const b0 = str.charCodeAt(i2++) & 255;
    const b1 = i2 < str.length ? str.charCodeAt(i2++) & 255 : 0;
    const b2 = i2 < str.length ? str.charCodeAt(i2++) & 255 : 0;
    const hasB1 = i2 - 2 <= str.length;
    const hasB2 = i2 - 1 <= str.length;
...
var fetchPolymarketEvents = (requester, apiUrl) => {
  const response = requester.sendRequest({ url: apiUrl, method: "GET" }).result();
  if (!ok(response)) {
  ...
  return raw.filter((ev) => Array.isArray(ev.markets) && ev.markets.length > 0).filter((ev) => Number(ev.volume ?? 0) >= 5000).slice(0, 8).map((ev) => {
    let yesPrice = 0.5;
    try {
      const prices = JSON.parse(ev.markets[0]?.outcomePrices ?? "[0.5,0.5]");
...
var runGroqAnalysis = (requester, input) => {
...
Required JSON shape:
{
  ...
  const completion = json(response);
  const content = completion?.choices?.[0]?.message?.content ?? "{}";
  return JSON.parse(content);
};
var onCronTrigger = (runtime2) => {
  const { polymarketApiUrl, groqApiUrl, groqModel, portfolio } = runtime2.config;
  ...
  const analysis = httpClient.sendRequest(runtime2, runGroqAnalysis, consensusIdenticalAggregation())(analysisInput).result();
